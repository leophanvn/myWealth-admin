...
module.exports = withBundleAnalyzer(withAntdLess({
	...
	// optional https://ithub.com/webpack-contrib/css-loader#object
	cssLoaderOptions: {
		modules: {
	...
	webpack(config) {
		...
		const { NEXT_RUNTIME, __NEXT_REACT_ROOT, ...rest } = process.env;

		config.plugins.push(
			new webpack.EnvironmentPlugin({ ...rest, 'THEME': { ...antdVariables } }),
		);
